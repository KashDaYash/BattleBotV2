...
const MONSTERS = [
  ...
  { name: "Rattle",  minLvl: 2, hp: 50,  atk: 12, def: 3, xp: 25, coins: 20, image: "/images/Rattle.png" },
  { name: "Gronk",   minLvl: 2, hp: 60,  atk: 15, def: 4, xp: 30, coins: 25, image: "/images/Gronk.png" },

  // TIER 2: MID LEVEL (Levels 4-9)
  { name: "Zombino", minLvl: 4, hp: 90,  atk: 20, def: 5, xp: 50, coins: 40, image: "/images/Zombino.jpg" },
  { name: "Venmora", minLvl: 5, hp: 100, atk: 22, def: 6, xp: 60, coins: 50, image: "/images/Venmora.jpg" },
  { name: "Floraxa", minLvl: 6, hp: 110, atk: 18, def: 8, xp: 55, coins: 45, image: "/images/Floraxa.jpg" },
...
export default async function handler(req, res) {
  ...
  if (action === 'search') {
    ...
    // Random HP Calculation
    const finalHp = Math.floor(baseEnemy.hp * (0.9 + Math.random() * 0.2));

    const enemy = { 
        ...baseEnemy, 
        hp: finalHp,
        maxHp: finalHp // 🔥 CRITICAL FIX: Ye line missing thi, ab bar 100% work karegi
    };
  ...
  if (action === 'attack') {
    ...
    if (e_hp <= 0) { 
      ...
      await db.collection("users").updateOne({ telegramId }, { 
          ...
          $set: { "character.xp": newXp, "character.level": newLevel, "character.xpToNext": newXpToNext, "character.stats": newStats }
      }); 
    } 
    else {
      // Enemy Turn
