...
// 🦖 Monster Database (Levels 1 to 15+)
const MONSTERS = [
  // TIER 1: EASY (Levels 1-3)
...
export default async function handler(req, res) {
  ...
  // --- SEARCH LOGIC ---
  if (action === 'search') {
    // Player ke level ke hisab se monster filter karo
    let availableMonsters = MONSTERS.filter(m => m.minLvl <= player.level);
    
    // Agar koi match na ho (Level 1 case), to sabse weak wala do
  ...
  if (action === 'attack') {
    ...
    if (e_hp <= 0) { 
      ...
      // 3. ENEMY ATTACKS
      const isDodged = Math.random() < 0.1; // 10% Dodge
      if (isDodged) {
